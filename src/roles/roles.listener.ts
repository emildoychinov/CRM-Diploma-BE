  ...
  constructor(
    ...
    this.roleService.findAll().then((roles: any) => {
      roles.forEach((role: any) => {
        ...
        this.queueService.createProcess(
          queue,
  ...
  async addToEntityProcess(
    ...
    switch (entityService) {
      case EntityService.OPERATOR:
        ...
        addRole = (role: Role, entity: Operator) =>
          role.operators?.push(entity);
        break;
      default:
    ...
    try {
      ...
      if (entitiesDto && entitiesDto.length > 0) {
        const promises = entitiesDto.map(async (entity: any) => {
          ...
          else roleEntity = await service.addRole(entity.id, role);
